    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated:
            if not hasattr(request.user, 'logged_in_user'):
                ...
                logged_in_user = LoggedInUser.objects.create(user=request.user, session_key=request.session.session_key)
...
class RSAMiddleware:
    ...
    def __call__(self, request):
        ...
        if 'private_key' not in request.session:
            ...
            request.session['private_key'] = base64.b64encode(private_key_pem).decode('utf-8')
            request.session['public_key'] = base64.b64encode(public_key_pem).decode('utf-8')
